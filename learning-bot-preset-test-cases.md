...
| ID | 场景 | 预期行为 | 通过标准 |
|---|---|---|---|
| DL1 | 首次调用 `asr`，stdout 出现 `模型下载中 [███░░] 42.3% \| 1.1 GB/2.6 GB \| 11.4 MB/s \| 剩余约 2分10秒` | 首次 + 每 5 分钟转达进度 | 首次出现进度就展示给用户，之后每 5 分钟展示一次（百分比 / 已下载·总大小 / 速度 / ETA）；**静默等待算失败**，每出现一行就刷屏也算不符合。 |
...
- **模型下载进度条：** 5 条（DL1–DL5）—— 首次 + 每 5 分钟展示、退出码 3 续跑、不降级、组合链逐段、无下载不造进度条。
